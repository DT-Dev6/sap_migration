...
def get_columns():
    return [
        ...
        {"label": "Vendor No", "fieldname": "vendor_number", "fieldtype": "Data", "width": 140},
        {"label": "Vendor Name", "fieldname": "vendor_name", "fieldtype": "Data", "width": 140},
...
def get_data(filters):
    ...
    query = f"""WITH Vendor_Line_Items AS (
        -- Get OPEN Items from BSIK
        SELECT 
            MANDT, BUKRS, LIFNR, AUGDT, AUGBL, ZUONR, GJAHR, BELNR, BUZEI, 
    ...
    SELECT 
        ...
        -- Handle Signs: In AP, Credits (H) are liabilities/invoices, Debits (S) are payments.
        -- This standardizes Credits as negative and Debits as positive.
        CASE 
    ...
    -- Join with Vendor Master Data for Names
    LEFT JOIN `tabmpr-LFA1` v
        ON  i.MANDT = v.MANDT
        AND i.LIFNR = v.LIFNR

    -- Filter by Date Range (Equivalent to FBL1N Selection Screen)
